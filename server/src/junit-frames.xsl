...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="directory">
		...
		<!--Processing directory <xsl:value-of select="@name"/> in <xsl:value-of 
			select="$output.dir"/> -->
		...
		<redirect:write
			file="{$output.dir}/{$directory.dir}/directory-summary.html">
		...
		<!-- @bug there will be a problem with inner projects having the same name, 
		...
		<xsl:for-each select="/testsuites/testsuite[@package = $name]">
			...
			<xsl:if test="failures/text() != 0">
				<redirect:write
					file="{$output.dir}/{$directory.dir}/{@id}_{@name}-fails.html">
					<xsl:apply-templates select="." mode="project.details">
						<xsl:with-param name="type" select="'fails'" />
					</xsl:apply-templates>
				</redirect:write>
			...
			<xsl:if test="errors/text() != 0">
				<redirect:write
					file="{$output.dir}/{$directory.dir}/{@id}_{@name}-errors.html">
					<xsl:apply-templates select="." mode="project.details">
						<xsl:with-param name="type" select="'errors'" />
					</xsl:apply-templates>
				</redirect:write>
	...
	<!-- ====================================================================== 
		This page is created for every testsuite project. It prints a summary of 
		the testsuite and detailed information about testcase methods. ====================================================================== -->
        ...
        function displayProperties (name) {
          ...
          win.focus();
			...
			<body>
				...
				<!--div class="Properties"> <a> <xsl:attribute name="href">javascript:displayProperties('<xsl:value-of 
					select="@package"/>.<xsl:value-of select="@name"/>');</xsl:attribute> Properties 
					&#187; </a> </div> <xsl:if test="string-length(./system-out)!=0"> <div class="Properties"> 
					<a> <xsl:attribute name="href">./<xsl:value-of select="@id"/>_<xsl:value-of 
					select="@name"/>-out.html</xsl:attribute> System.out &#187; </a> </div> </xsl:if> 
					<xsl:if test="string-length(./system-err)!=0"> <div class="Properties"> <a> 
					<xsl:attribute name="href">./<xsl:value-of select="@id"/>_<xsl:value-of select="@name"/>-err.html</xsl:attribute> 
					System.err &#187; </a> </div> </xsl:if -->
			</body>
		</html>
	</xsl:template>
	...
	<!-- ====================================================================== 
		This page is created for every directory. It prints the name of all projects 
		that belongs to this directory. @param name the directory name to print projects. 
		====================================================================== -->
		...
		<tr>
			<td nowrap="nowrap">
				<a target="projectFrame">
                    ...
                    <xsl:if test="not($directory.name='')">
                        <xsl:value-of
						select="translate($directory.name,'.','/')" /><xsl:text>/</xsl:text>
	...
	<!-- Creates an html file that contains a link to all directory-summary.html 
		files on each directory existing on testsuites. @bug there will be a problem 
		here, I don't know yet how to handle unnamed directory :( -->
			...
			<body>
				...
				<table class="details" border="0" cellpadding="5" cellspacing="2"
					...
					<xsl:for-each
                        ...
                        <xsl:choose>
                            <xsl:when
								test="sum($insamedirectory/errors/text()) &gt; 0">Error</xsl:when>
                            <xsl:when
								test="sum($insamedirectory/failures/text()) &gt; 0">Failure</xsl:when>
                            <xsl:otherwise>Pass</xsl:otherwise>
                        </xsl:choose>
                    </xsl:attribute>
							<td>
							...
							<td>
								<xsl:call-template name="display-time">
									<xsl:with-param name="value"
										select="sum($insamedirectory/time/text())" />
								</xsl:call-template>
							</td>
							<td>
	...
	<xsl:template name="directory.summary">
		...
		<html>
			...
			<body>
				...
				<!--table border="0" cellpadding="5" cellspacing="2" width="95%"> <xsl:call-template 
					name="project.metrics.header"/> <xsl:apply-templates select="." mode="print.metrics"/> 
					</table -->
				...
				<xsl:if test="count($insamedirectory) &gt; 0">
					...
					<p>
						<table class="details" border="0" cellpadding="5"
							...
							<xsl:call-template name="testsuite.test.header" />
							<xsl:apply-templates select="$insamedirectory"
								mode="print.test">
								<xsl:sort select="@name" />
							</xsl:apply-templates>
						</table>
	...
	<xsl:template name="create.stylesheet.link">
		...
		<link rel="stylesheet" type="text/css" title="Style">
			<xsl:attribute name="href"><xsl:if
				test="not($directory.name = 'unnamed directory')"><xsl:call-template name="path"><xsl:with-param
				name="path" select="$directory.name" /></xsl:call-template></xsl:if>stylesheet.css</xsl:attribute>
		</link>
	</xsl:template>
	...
	<xsl:template name="br-replace">
		...
		<xsl:choose>
			<xsl:when
				test="(string-length($word) > $splitlimit) and (contains($secondhalfword, '&#xa;'))">
				<xsl:variable name="secondhalfend"
					select="substring-after($secondhalfword, '&#xa;')" />
				<xsl:variable name="firsthalflen"
					select="string-length($word) - $secondhalflen" />
